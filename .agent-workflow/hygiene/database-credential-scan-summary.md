...
- These eight are reviewed test/schema-shaped false positives already present at the approved baseline.
...
- A separate boolean-only live comparison established that a committed standalone-client fallback matched the still-valid production application DB credential. That is the reason rotation remains a release-blocking protected gate.
...
3. Do not release the recovery wave until the production rotation gate is explicitly authorized and closed with new-valid/old-invalid boolean evidence.
