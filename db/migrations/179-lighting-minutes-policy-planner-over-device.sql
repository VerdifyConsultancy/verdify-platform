-- 179-lighting-minutes-policy-planner-over-device.sql
-- Generalize the migration-176 lux feedback-loop fix to EVERY lighting param.
--
-- 176 fixed only the LUX columns (via the fn_lighting_circuit_policy lateral
-- join, device-readback excluded). But the latest_values CTE still ranked the
-- device cfg readback (latest_snapshot, source_rank 1) ABOVE the planner/operator
-- (latest_confirmed_changes, source_rank 2) for every OTHER lighting param
-- (target_light_minutes, dli_target, sunrise/sunset hour, min_on/off, auto-mode).
-- So after a reboot reverted a firmware global to its cold default, the device
-- snapshot outranked the confirmed planner change and the dispatcher re-pushed
-- the stale value — the same poison-loop 176 killed for lux, still live for the rest.
--
-- Fix: swap the two source_rank values so latest_confirmed_changes (planner/
-- operator, rank 1) outranks latest_snapshot (device readback, rank 2) for ALL
-- params. latest_switch_actual (the real switch state, rank 0) stays highest and
-- the lux lateral join is unchanged. The function is otherwise byte-identical to
-- 176 (only the two source_rank literals differ).
--
-- Non-self-transactional (CREATE OR REPLACE, no top-level COMMIT/CONCURRENTLY):
-- safe to rollback-validate by wrapping in BEGIN; ... ROLLBACK;.
