...
--
-- Migration 236 fixed pre-claim ordering, but still required the replacement
-- writer's current-lease recovery to fall inside the immutable predecessor
-- proof authorization.  A later attended PostSync recovery cannot satisfy both
-- the expired predecessor range and the current-lease gate.  Bind the reset and
-- recovered receipt pair to their own exact bounded five-minute recovery range
-- instead.  The caller remains attended-window gated, the recovery stays on the
-- current revision/lease, aggressive exposure is forbidden, and this recovery
-- is resolution evidence only--never physical proof evidence.
...
BEGIN
    ...
    SELECT recovery.* INTO v_recovery
     ...
     WHERE fault.experiment_id = p_experiment_id
       ...
       AND recovery.created_at >= v_aggressive_created_at
