-- 206-water-est-completed-day-guard.sql
--
-- Amendment to migration 204's ungated estimate columns: the raw-meter
-- fallback in water_gal_est / cost_water_est / cost_total_est also fired
-- for the CURRENT (partial) day, where the water ledger is by definition
-- unvalidated ('partial_day') and raw water_used_gal can carry
-- reset/discontinuity double-counts — observed live 2026-07-15: raw meter
-- 1388 gal intraday vs a 78–346 gal/day normal range, which would have
-- inflated the 30-day water totals/averages on site-home the moment the
-- panels reloaded.
--
-- Guard: the quality-filtered value still serves any day water_ok is true
-- (including today); the RAW fallback now only serves COMPLETED days
-- (date < current_date), where at worst it matches what the legacy
-- dashboards always showed. Today's water panels show no value until
-- either the ledger validates intraday or the day completes.
--
-- SELF-TRANSACTIONAL (own BEGIN/COMMIT): rebuilds mv_daily_kpi atomically,
-- same as migration 204. Do NOT wrap in an outer BEGIN..ROLLBACK;
-- rollback-validate by swapping the trailing COMMIT for ROLLBACK.
-- Functional rollback: re-apply migration 204's view body + matview.
...
'Dashboard/scorecard snapshot of v_daily_kpi (migrations 203/204/206), '
'refreshed every 10 min by the verdify-band-curve-refresh CronJob. '
'fn_planner_scorecard and panels must read THIS, not the live view — 27 '
'live-view scans per scorecard call OOM-crashed the DB on 2026-07-14/15. '
'The *_est/*_modeled columns are ungated display estimates (raw-meter water '
'fallback restricted to completed days per migration 206); the gated '
'kwh/cost_* columns remain the ADR-0004 evidence tier.';
