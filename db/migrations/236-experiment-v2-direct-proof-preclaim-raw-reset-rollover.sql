...
--
-- Migration 235 correctly isolated the raw-reset/root-recovery lineage, but it
-- accidentally required the reset fault to be newer than an aggressive-open
-- event.  The production race is earlier: BEGIN has durably created and bound
-- the aggressive work, then the raw monitor records its fault before any claim,
-- exposure, or open transition.  Bind time ordering to the aggressive work's
-- immutable creation time, require that work to have no exposure row at all,
-- and retain the global zero-open-exposure gate.  The recovered baseline is
-- resolution evidence only and is never credited as physical proof evidence.
...
DECLARE
    ...
    v_aggressive_created_at timestamptz;
...
BEGIN
    ...
    SELECT aggressive.created_at INTO v_aggressive_created_at
      FROM public.experiment_v2_work aggressive
     WHERE aggressive.experiment_id = p_experiment_id
       AND aggressive.work_id = v_aggressive_work_id;
    SELECT recovery.* INTO v_recovery
     ...
     WHERE fault.experiment_id = p_experiment_id
       ...
       AND fault.recorded_at > v_aggressive_created_at
    ...
    IF v_exp.experiment_id IS NULL OR
       ...
       v_aggressive_created_at IS NULL OR
       v_recovery.work_id IS NULL OR
       v_fault_at IS NULL OR
       v_recovered_at IS NULL OR
       NOT (v_aggressive_created_at < v_fault_at AND v_fault_at < v_recovered_at) OR
       ...
       EXISTS (
           SELECT 1
            ...
            WHERE exposure.experiment_id = p_experiment_id
              AND exposure.work_id = v_aggressive_work_id) OR
       EXISTS (
           SELECT 1
             FROM public.experiment_v2_exposures exposure
    ...
    SELECT count(DISTINCT receipt.receipt_id)::integer,
           encode(digest(convert_to(
               'verdify-direct-proof-startup-raw-reset-v2|' ||
