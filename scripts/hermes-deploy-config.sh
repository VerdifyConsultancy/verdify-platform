...
SLACK_SECRET_DIR="${VERDIFY_SLACK_SECRET_DIR:-/etc/verdify/slack}"
...
sudo install -m 600 -o "$RUNTIME_UID" -g "$RUNTIME_GID" \
  "$ROOT/slack.yaml" "$RUNTIME_DIR/slack.yaml"

for token_file in \
  "$SLACK_SECRET_DIR/iris_slack_bot_token.txt" \
  "$SLACK_SECRET_DIR/iris_slack_app_token.txt"
do
  if ! sudo test -r "$token_file"; then
    echo "Warning: Slack token file is not readable yet: $token_file" >&2
  fi
done

echo "Hermes config synced to $RUNTIME_DIR"
echo "Hermes env file present at $ENV_FILE"
echo "Hermes expects Slack token files under $SLACK_SECRET_DIR"
