...
WITH periods AS (
  ...
  SELECT
    ...
    ds.kwh_estimated,
    ds.cost_total,
    -- This fixed April/May receipt predates score-eligible resource provenance.
    -- v_planner_performance therefore takes its climate-only branch for every
    -- row: COALESCE(attributable compliance, binary compliance, 0).  Query that
    -- branch directly instead of expanding the resource-accounting views across
    -- the entire telemetry history; that expansion exceeds the publisher's
    -- bounded statement timeout even though only eleven daily rows are needed.
    COALESCE({graded_day_expr}, ds.compliance_pct, 0) AS planner_score,
...
SELECT
  ...
  round(COALESCE({graded_day_expr}, ds.compliance_pct, 0)::numeric, 1) AS planner_score,
...
<div class="data-table">
  ...
  <div class="data-row"><strong>Planner score</strong><span><code>daily_summary.compliance_v2_attributable_pct</code> (binary fallback)</span><p>These fixed historical rows predate score-eligible water and energy provenance, so the planner KPI gives resource terms zero weight and uses climate compliance only. It is useful as an operational KPI, not as a yield claim.</p></div>
...
This page is generated by `scripts/generate-baseline-vs-iris-page.py` from `daily_summary`, `plan_journal`, `climate`, and `crop_events`.
