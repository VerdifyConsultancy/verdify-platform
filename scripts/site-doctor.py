...
# The AI tunables page regenerates on every plan publish (publish-site-content.sh),
# and full-plan triggers fire several times per local day. Its frontmatter carries
# only a date-only `date` field, so the freshness gate is intentionally coarse: a
# date more than ~2 days stale means the publish pipeline has stopped refreshing it.
AI_TUNABLES_MAX_AGE_SECONDS = 2 * 24 * 60 * 60
...
def check_ai_tunables_freshness(vault_root: Path) -> list[Finding]:
    """Flag a stalled AI-tunables-page publish.

    The page at reference/ai-tunables.md is regenerated by
    scripts/generate-ai-tunables-page.py and pushed through
    scripts/publish-site-content.sh on every plan publish. Its frontmatter `date`
    field is the only freshness signal it carries; if it falls behind, the
    registry/clamp/trigger evidence on the public planner contract has gone stale.
    """
    findings: list[Finding] = []
    rel = "reference/ai-tunables.md"
    path = vault_root / rel
    if not path.exists():
        findings.append(Finding("error", "ai-tunables-page-missing", f"{rel} is missing"))
        return findings
    text = read_text(path)
    if "auto-generated by scripts/generate-ai-tunables-page.py" not in text:
        findings.append(
            Finding(
                "error",
                "ai-tunables-marker-missing",
                f"{rel} has no generate-ai-tunables-page.py marker; it may have been hand-edited or replaced",
            )
        )
    fm, _body = frontmatter(text)
    raw = clean_scalar(fm.get("date"))
    if not raw:
        findings.append(Finding("error", "ai-tunables-freshness-missing", f"{rel} has no date frontmatter"))
        return findings
    try:
        updated_at = datetime.fromisoformat(raw)
    except ValueError:
        findings.append(Finding("error", "ai-tunables-freshness-invalid", f"{rel} has invalid date: {raw}"))
        return findings
    if updated_at.tzinfo is None:
        updated_at = updated_at.replace(tzinfo=DENVER_TZ)
    age_seconds = (datetime.now(updated_at.tzinfo) - updated_at).total_seconds()
    if age_seconds > AI_TUNABLES_MAX_AGE_SECONDS:
        findings.append(
            Finding(
                "error",
                "ai-tunables-page-stale",
                f"{rel} date is {int(age_seconds // 3600)}h old; "
                "publish-site-content.sh should refresh it on every plan publish",
            )
        )
    return findings
...
def main() -> int:
    ...
    findings.extend(check_ai_tunables_freshness(args.vault_root))
