...
Checks:
  ...
  5. Every event-type prompt builder renders a non-empty string for BOTH
     instances (opus, local). Assertions:
      - opus and local audit labels render the same Hermes/Cortex prompt.
...
assert opus_preamble == local_preamble, "Hermes uses one Cortex prompt for both audit labels"
