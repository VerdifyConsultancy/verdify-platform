...
type SiteNavItemOptions = Pick<SiteNavItem, "activePrefixes" | "aliases" | "exact">
...
function pageLink(label: string, slug: string, options: SiteNavItemOptions = {}): SiteNavItem {
  return {
    label,
...
function latestPlanLink(allFiles: QuartzComponentProps["allFiles"]): SiteNavItem {
  const slug = allFiles
  ...
  return {
    groups: [
      ...
      },
      {
        title: "Reference",
        links: [
          pageLink("Baseline vs AI Planning Agent", "data/baseline-vs-iris"),
          pageLink("Planning Loop", "reference/planning-loop"),
          pageLink("Planner Contract and AI Tunables", "reference/ai-tunables", {
...
function isActiveBranch(currentSlug: string, item: SiteNavItem): boolean {
  return (
    ...
    Boolean(item.children?.some((child) => isActiveBranch(currentSlug, child)))
  )
}

function activeSectionLabel(currentSlug: string, model: SiteNavModel) {
  return (
    model.groups.find((group) => group.links.some((link) => isActiveBranch(currentSlug, link)))
      ?.title ?? "Menu"
  )
}
...
function renderItem(currentSlug: string, item: SiteNavItem, child = false) {
  ...
  if (item.children?.length) {
    return (
      <li
        class={
          child ? "site-nav__child site-nav__branch-item" : "site-nav__item site-nav__branch-item"
        }
      >
        <details class="site-nav__branch" open={branchActive}>
          ...
          <ul class="site-nav__children">
            {item.children.map((nested) => renderItem(currentSlug, nested, true))}
          </ul>
        </details>
      </li>
    )
...
function renderNavPanel(currentSlug: string, _idPrefix: string, model: SiteNavModel) {
  return (
    <div class="site-nav__panel">
      {model.groups.map((group) => (
        <details class="site-nav__group" open>
          ...
          <ul>{group.links.map((link) => renderItem(currentSlug, link))}</ul>
        </details>
      ))}
    </div>
  )
...
const SiteNav: QuartzComponent = ({ fileData, allFiles }: QuartzComponentProps) => {
  ...
  return (
    <nav class="site-nav" aria-label="Site navigation">
      <div class="site-nav__desktop">{renderNavPanel(currentSlug, "site-nav-desktop", model)}</div>
