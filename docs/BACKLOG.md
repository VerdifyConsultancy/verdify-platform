...
- **Planner:** Hermes hardening is now the top planning backlog. Genai owns the GPT-5.5 context pack, tuning rubric, site/lesson digest, MCP `plan_run` audit parity, and registry validation at the planner boundary. Ingestor owns the remaining trigger matrix and planner health status surface; expected required-trigger ledgering, per-trigger SLA lifecycle, exact trigger correlation, fixed-boundary triggers, active/future plan range guard, and forecast deviation coverage are deployed. Coordinator owns shared contracts, trigger-ledger schema, and model observability.
