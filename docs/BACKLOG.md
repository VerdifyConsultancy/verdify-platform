...
> (sub-issues: #286→#287/#288, #287→#289–#300, #288→#301–#307).
> Written replan: [`docs/verdify-backlog-replan-2026-06-09.md`](verdify-backlog-replan-2026-06-09.md);
> control diagnoses: [`docs/voice-memo-triage-2026-06-09.md`](voice-memo-triage-2026-06-09.md).
> Greenhouse control = epic **#287** (req A–E); deploy/agent enablement = **#288**.
> The per-agent `docs/backlog/*.md` files below predate this replan (historical).
...
| `ingestor` | planner hardening in progress | Hermes delivery is active, trigger ledger rows carry Hermes run IDs, the canonical Phase 4 trigger matrix drives active solar, solar-peak, forecast-deviation, manual, and short-window midnight cycles, per-trigger SLA timeouts alert through `planner_trigger_sla_timeout`, expected required triggers alert through `planner_required_plan_missed`, active/future plan range drift is guarded through `planner_tunable_range_drift`, forecast deviation triggers cover the planner-critical weather axes and queue through typed `AlertEnvelope` rows, and the public planner-health API now exposes last trigger lifecycle, pending SLA-age buckets, current session/model labels, and active-plan range violations. Remaining ingestor backlog is outside this planner-hardening P0/P1 surface. | `docs/backlog/ingestor.md` |
...
- **Planner:** Hermes hardening is now the top planning backlog. Genai owns the GPT-5.5 context pack, tuning rubric, site/lesson digest, MCP `plan_run` audit parity, and registry validation at the planner boundary. Expected required-trigger ledgering, per-trigger SLA lifecycle, exact trigger correlation, trigger matrix coverage, active/future plan range guard, forecast deviation coverage, public planner health, and model/session observability are deployed. Coordinator still owns any future shared contract or context-digest schema changes.
...
- Expected required-trigger ledgering catches missed startup windows through `planner_required_plan_missed`
- Forecast deviation triggers cover temp, RH/VPD, solar, wind, precipitation/cloud, and missing-forecast axes
- Remaining: canonical trigger matrix and planner health status surface
...
Remaining high-leverage work: canonical trigger matrix, model/status surface, and distilled site/lesson context digest.
