...
| `ingestor` | planner hardening in progress | Hermes delivery is active, trigger ledger rows carry Hermes run IDs, the canonical Phase 4 trigger matrix drives active solar, solar-peak, forecast-deviation, manual, and short-window midnight cycles, per-trigger SLA timeouts alert through `planner_trigger_sla_timeout`, expected required triggers alert through `planner_required_plan_missed`, active/future plan range drift is guarded through `planner_tunable_range_drift`, and forecast deviation triggers cover the planner-critical weather axes. Remaining: planner health status surface | `docs/backlog/ingestor.md` |
