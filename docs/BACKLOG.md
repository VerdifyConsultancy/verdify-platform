...
| `ingestor` | planner hardening in progress | Hermes delivery is active, trigger ledger rows carry Hermes run IDs, fixed 00:00/06:00/12:00/16:00/20:00 boundary triggers supplement sunrise/sunset, forecast, deviation, and solar-derived transition triggers, per-trigger SLA timeouts alert through `planner_trigger_sla_timeout`, expected required triggers alert through `planner_required_plan_missed`, active/future plan range drift is guarded through `planner_tunable_range_drift`, and forecast deviation triggers cover the planner-critical weather axes. Remaining: canonical trigger matrix and planner health status surface | `docs/backlog/ingestor.md` |
