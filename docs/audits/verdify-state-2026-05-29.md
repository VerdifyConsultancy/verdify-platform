...
- **Surface clamp/rejection events to OPERATORS** (page column + Grafana + Slack) and gate band params at the MCP write layer → the planner **already** receives live clamp feedback each cycle (`gather-plan-context.sh:998-1009` queries `setpoint_clamps`), so the gap is human-facing, not planner-facing: `v_clamp_activity_24h` exists and nothing reads it, `mister_engage_kpa` was clamped ~681x/30d and the `vpd_high_moisture_guardrail` clamped Iris ~2788x/30d — all invisible to humans. Stop silently dropping operator irrig_* params at the MCP write layer too. **(This sprint implements the operator-facing exposure.)**
