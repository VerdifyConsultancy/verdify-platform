...
## The corpus-wide guard can wedge every publish (2026-07-26)

`check-public-output.py` scans the **whole** content corpus and
`publish-site-content.sh` treats it as a promotion prerequisite, but the flow
only regenerates `PREV_DATE` and `DATE`. So a defect in *any* older generated
page blocks *every* future publish, and nothing in the pipeline can heal it.

That is exactly what happened. `f17e30f` (2026-07-12) added the guard **and**
fixed the two defects it detects — non-public crop redaction in
`public_text()`, and the `USD None` currency rendering. Production's publisher
image was never advanced past 2026-07-11, so 2026-07-12 → 07-25 content was
generated by the pre-fix generator and published ungated. The moment a run used
the fixed image the guard fail-closed on its own predecessor's output:

```text
public-output guard: 22 prohibited artifact finding(s)
[...] publish finished with 1 failed step(s) after retries; rebuild/promotion blocked
```

Compounding it: the cache PVC was recreated 2026-07-26T09:35Z, so the
`verdify-lab` pod re-seeded from the **baked fallback** in the `verdify-lab`
image — which is Quartz's own upstream documentation build, not this site. The
content-policy scanner passes that tree (nothing in it is prohibited, it is
simply the wrong site), so lab.verdify.ai served "Welcome to Quartz 4".
`prepare-lab-cache.sh` now checks bootstrap *identity* (`is_lab_site_tree`) and
fails the rollout rather than seeding a foreign tree.

When the guard blocks promotion, read the report and repair the **corpus**, not
the guard:

```bash
kubectl -n verdify-prod exec <inspect-pod> -- \
  check-public-output --root /work/publisher/content --json-report /tmp/r.json
```

Findings name a `reason` per route. `invalid-rendered-value` / `content` in old
generated pages mean legacy artifacts that the current generator would emit
correctly — repair them in S3 (`content/`) so every later run inherits the fix;
regenerating only today's page will not clear them. A genuinely new finding
