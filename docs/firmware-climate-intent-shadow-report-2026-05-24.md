# ClimateIntent Shadow Replay Report - 2026-05-24
...
```bash
make climate-intent-shadow-report
```
...
hot_dry_rows=19615
wet_relay_rows_when_shadow_blocked=2423
shadow_action_transitions=15669
firmware_state_transitions=18879
resource_estimate={"electric_kwh": 207.99, "gas_therm": 84.532, "water_gal": 1307.96}
shadow_action_counts={"DEHUM_VENT": 6694, "HEAT": 42266, "IDLE": 97669, "SAFETY_COOL": 695, "SEALED_FOG": 3403, "SEALED_HUMIDIFY": 20942, "VENT_COOL": 5372, "VENT_COOL_FOG_ASSIST": 12857, "VENT_COOL_MIST_ASSIST": 3627}
...
The first shadow selector is intentionally conservative and deterministic:
...
- The shadow action stream has fewer transitions than the historical firmware
...
- `2,423` rows had wet relays on while the shadow model would report moisture
...
- This is a replay-only shadow model; it does not yet share code with firmware.
...
1. Add the same candidate projection structs to firmware behind a feature flag.
2. Publish shadow `climate_action`, `priority_axis`, block reasons, and timers
   without changing relay behavior.
3. Feed live forecast segments into `ClimateIntent` generation.
4. Compare shadow decisions against live outcomes for 14-21 days before any
   behavior-changing OTA.
