# Verdify Platform — k3s Operations

This runbook records the k3s operating model, the development loop runnable from
any kubectl-equipped host, repository sources of truth, firmware OTA operations,
infrastructure dependencies, and durable operational gotchas.

Use this with `README.md`, `ARGOCD.md`, and the focused runbooks linked below.

---

## 1. Operating model

- Work from the repository with kubectl, the in-cluster DB, and scoped runtime
  credentials. Keep code, schemas, migrations, CI, k8s manifests, Grafana
  dashboards, and docs aligned on **`main`**.
- Live firmware, ArgoCD, database, credential, and public-edge changes require
  an explicit live command, scoped credentials, passing preflight, observable
  verification, and a retained rollback position.
...
Everything below works from a kubectl-equipped pod or host.
...
- **Deploy / GitOps (one greenhouse environment, plus an isolated static Lab
  ...
  kubectl patch application verdify-prod-dark -n argocd --type merge \
    -p '{"operation":{"initiatedBy":{"username":"<operator>"},"sync":{"prune":false}}}'
  ```
...
## 4. Firmware OTA (safety-checked)

The OTA path requires the toolchain, device-VLAN reachability, and scoped
secrets. The procedure and its traps:
...
- **The preflight checks are mandatory** (`scripts/firmware-deploy-preflight.sh`): no OTA while
...
These items record remaining infrastructure work and completed portability
milestones:
...
6. **Durable context** — operational knowledge belongs in this runbook and the
   focused `docs/` set rather than workstation-local memory.
...
- **This runbook** + `README.md` + `ARGOCD.md` — start here.
...
## 8. Recorded state snapshot (2026-06-18)
