...
Status: **COMPLETE**. The `rotate-now` run completed on 2026-07-10.
...
Execution evidence: `docs/security/database-credential-rotation-closeout-2026-07-10.md`
...
1. The exact target and `rotate-now` action are explicit in the execution request.
...
4. No CronJob using the password is running; the five listed CronJobs are suspended for the scoped execution.
5. The operator has a secure local shell with tracing/history disabled, `umask 077`, the fleet SOPS age key, the current credential retained only for rollback/negative proof, and the replacement held only in the configured credential manager or protected process memory.
6. The fleet secret authority is verified before use. At the 2026-07-09
   ...
   `verdify-staging`; that historical blocker was corrected by the production
   SOPS authority in `jvallery/agents#2904`. For every future execution, a
   validated production-targeted artifact for
...
11. **Prove new-valid.** From an isolated secure operator path, test a new database connection and every consumer outcome. Store booleans/statuses only. Application health without an authenticated query is insufficient where a targeted read can be made safely.
...
13. **Close out.** Confirm all intended workloads are Ready, no auth failures are rising, ingestor remains the only writer, scheduled proofs passed, the current Secret/SOPS metadata point at production, and rollback material is dispositioned. Record immutable refs and boolean evidence.
...
5. Re-run old-valid health checks, record the rollback verdict, and mark the rotation incomplete.
