...
The runtime prompt is split into two layers for the live Hermes/Cortex `custom:llm.primary.longctx` medium-reasoning profile, keeping stable core instructions separate from longer reference material. The profile has a 98,304-token context contract and a 16,384-token output fence.
