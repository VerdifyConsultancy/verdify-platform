...
The runtime prompt is split into two layers for the repo-selected Hermes profile routed through Cortex alias `llm.primary.longctx.mm`, keeping stable core instructions separate from longer reference material. Live route activation is a separate gated step.
