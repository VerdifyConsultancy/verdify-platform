...
- **Planner:** Iris currently uses Hermes to choose tactical setpoints and plans through MCP tools. The live profile is OpenAI GPT-5.6 Sol with xhigh reasoning.
...
- Some older docs mention OpenClaw/local Gemma. Current production planning uses Hermes `hermes-iris` on OpenAI GPT-5.6 Sol with xhigh reasoning.
