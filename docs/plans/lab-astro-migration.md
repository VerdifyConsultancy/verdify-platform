...
The source tree also defines a dedicated offline-default
`site-astro/Dockerfile.occurrence-exporter` image build;
...
1. **4c Graphs/camera producer first:** build the isolated reporting/export
   ...
   The source tree now also defines a dedicated `occurrence-exporter` Dockerfile
   derived from the locked dependencies image. It copies only two additional project-source
