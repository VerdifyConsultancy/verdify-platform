# Band-Compliance Build — OTA Sign-Off Receipts (2026-06-17)

**Milestone:** the band-compliance firmware (WS-A–D) is **implemented, proven offline, and
STAGED — ready for OTA on Jason's command.** Nothing is deployed; the OTA flash and the
gated `argocd` data/web sync are Jason's gates per the freeze rules.
...
## 3. OTA-staging checklist (ready on Jason's command)
...
**On Jason's command only:** `make firmware-deploy` (preflight → compile → OTA → soak →
