...
| `verdify-lab-publisher` | `scripts/lab-publish-k3s.sh` | `deploy/k8s/components/lab-site/lab-publisher.yaml` | `ghcr.io/verdifyconsultancy/verdify-lab-publisher-k3s` | CronJob, no inbound service | S3 content/public/state store, TimescaleDB, Quartz build toolchain; writes only the lab cache PVC and S3 prefixes. |
| `verdify-grafana` | Grafana plus image-renderer sidecar | `deploy/k8s/components/grafana/grafana.yaml` | digest-pinned `grafana/grafana:12.4.3-security-02`, `grafana/grafana-image-renderer:v5.9.1` | `3000`, ClusterIP; prod `graphs.verdify.ai` | Provisioned dashboards/config from git; legacy application-credential datasource (not a reporting-only security boundary); required admin/database Secrets; non-default per-pod renderer capability value plus port isolation; surge-first rollout. |
