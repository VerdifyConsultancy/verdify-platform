...
The curve ships as a coordinator **DB migration (functions + data) + dispatcher/MCP restart**. The firmware already enforces whatever band the dispatcher pushes, so **no OTA, no firmware-replay-diff block, no 48h bake for the curve itself**. Validate the behavioral change (cooling onset at ~86-88F instead of 78F) with `make firmware-replay` evidence and a recorded `THRESHOLD_PCT` check. Effect: cooling-relay duty *drops* (fewer futile calls into 95F air), nights become *deeper and drier*, and compliance is measured against a band the box can occupy.
