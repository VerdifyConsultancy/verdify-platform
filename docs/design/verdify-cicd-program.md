...
> and validate the exact target.** Every step here is **additive + reversible**: k3s stands up
> *alongside* the running VM; the VM stays authoritative until a controlled, per-service cutover.
> Companion doc `docs/design/k3s-argocd-migration.md` is the prior repo-grounded planning
> pass; this doc is the gravity-shaped program that supersedes its structure. Device,
> secret, and cluster steps below use **[SAFEGUARD: exact-target execution]**: resolve the
> target, run the preflight, stage rollback, execute, and verify.
...
**name are FALSE on this box and on the GitHub remote**. Reconcile every branch reference
against the current remote before P2+ work. The drift *count* matches; the *name* does not.
...
- The 4 live-only commits are the Vanda work: `aa6518c` (sprint-3 close-out), `f2bad50`
  ...
  any reconciliation change carries the full firmware artifact set (replay-diff
  `THRESHOLD_PCT=0`, 16 invariants, and the test-firmware delta).
...
**RECONCILIATION ACTION [SAFEGUARD: exact-target execution]:** before designing against `main`, (a) confirm
...
or they silently resolve to nothing. Resolve the canonical remote ref first, preserve
unrelated branch work, and commit the corrected references directly.
...
- **Compose stack** (`docker-compose.yml`): `traefik`, `timescaledb`
  ...
  (profile-controlled, `127.0.0.1:8642`, reaches host MCP via `host.docker.internal`).
- **Deploy today = `git merge to live/platform-main` + manual `systemctl restart` /
  ...
  `ci.yml`** (8 validation jobs: lint, site-guards, schemas, firmware, firmware-logic,
...
device-affecting **[SAFEGUARD: exact-target execution]** — confirm rotate-at-seal vs carry-existing; never trigger
...
```
registry (jvallery/agent-fleet-control)        ← source of truth, validation-enforced
...
- **DEV-AGENT plane (VLAN 64, agents-pool `192.168.64.10-95.254`):** the dev pods. Registry
  ...
  actual registry entry names from the authoritative registry; the observed entry is
  `verdify-ingestor .64.32`.)
...
- apps-pool `autoAssign:false`, so the `metallb.universe.tf/loadBalancerIPs: 192.168.7.<x>`
  annotation is **required**; `externalTrafficPolicy: Local`. Resolve a free `.7.x` address
  inside `.7.10-.250` from the authoritative network registry (not `.7.20`, gravity's
  repo-side value), check for collisions, then commit it to the registry.

---

## 3. CI/CD wiring (the workflows to add)

Commit workflow, Dockerfile, and deploy edits directly in
`VerdifyConsultancy/verdify-platform`; commit the ArgoCD Application and registry source in
the owning jvallery repositories. Each repository's objective validation must pass before
the associated ArgoCD sync.
...
- **PRs build-without-publish** (the reusable workflow's `push_allowed` condition is
  `PUSH_REQUESTED=true && event != pull_request && branch in publish-branches`). Mandatory
  tag is `sha-<12char>`.
- **Keep `ci.yml` as the unit validation** (lint/test/firmware/replay/drift-guards). Issue #22:
...
detailed endpoint is the #1 cross-cutting requirement; keep tool semantics unchanged and
prove the wrapper with API tests).
...
| `verdify-ingestor` | `ESP32_API_KEY` (Noise PSK — **device-affecting [SAFEGUARD: exact-target execution]**), DB creds, `MQTT_PASS` | from `ingestor/.env` |
...
- **mcp:** add a transport auth guard that **fails CLOSED** — today there is none (the 18+
  ...
  anti-pattern). **WRAP without changing tool semantics**; any tool-surface edit requires
  the full firmware replay, invariant, and build artifact set.

### 4.4 Registry validation (both commands must exit 0)
...
1. **Source quiescent** — at a quiescent moment, stop DB writers cleanly (this is the only
   atomic-handoff point; do NOT let both stacks write the same DB) **[SAFEGUARD: exact-target execution]**.
...
## 6. The device-VLAN decision (§3.4 — the single biggest risk, CONTROLLED SPIKE)
...
### 6.3 The concrete provable spike [SAFEGUARD: exact-target execution]

Treat "can a k3s pod reach `192.168.10.111:6053` within the 5–10s SLA" as a **discrete,
controlled spike BEFORE committing to move the ingestor**. **Design only here; implement no
routing/firewall/NetworkPolicy egress change.** Run the spike from an exact, disposable pod:
...
3. **Egress design (paper only):** the NetworkPolicy egress allow for `192.168.10.0/24` +
   ...
   transfer). For a requested route change, snapshot the exact current policy, validate the
   target networks, apply the narrow rule, test reachability, and retain the rollback.
...
| `timescaledb` | **MOVE (controlled, copy-not-move)** | k3s StatefulSet, Retain PVC | system of record; §5 atomic handoff |
| `traefik` | **MOVE** | k3s apps ingress | already Traefik-shaped |
| `mosquitto` | **conditional** | k3s OR stays near device VLAN | §6 decision |
| **`ingestor` + dispatcher** | **STAYS VM-side initially; LAST/maybe-never** | VM systemd | device-touching, single-writer, §6 spike-controlled |
...
`greenhouse_logic.h`, `entity_map.py`, or `mcp/server.py` semantics without the full replay,
invariant, unit-test, and build artifact set.

---

## 8. The cutover validation + Definition of Done (11 points)
...
4. **Checks green:** registry `make validate` + `make verify-reproducible`, and app-repo
...
9. **Secrets SOPS-sealed:** every secret is a `registry/secrets/<id>.yaml` meta + age payload,
   ...
   runtime contract.** ESP32 PSK/OTA password handled per the device-write preflight.
...
## 9. P0–P9 phased plan (per-phase validation / rollback / exact-target preflight)

Additive throughout; the live VM keeps running until the final controlled cutover.
...
- **Validation:** drift reported (not auto-fixed); audit written. **Rollback:** n/a (read-only).
- **Exact-target action:** resolve the canonical branch with `git ls-remote`, then correct the
  handoff's `origin/live`/`vm-verdify` references before any P2+ step.

### P1 — Design (THIS DOC)
- **Steps:** gravity-shaped program written; §6 device-VLAN decision front and center
  (ingestor stays VM-side initially); DB runbook + cutover validation + DoD defined.
- **Validation:** the device-reachability spike is concretely provable and any
  firewall/routing implication has an exact target and rollback. **Rollback:** n/a.
- **Exact-target action:** run the read-only device-VLAN probe and decide from measured
  reachability and latency whether the ingestor moves.

### P2 — Registry change (jvallery/agent-fleet-control)
- **Steps:** source-field-only edits — APP cross-cutting contract (a registry entry with
  ...
  `make transform`, validate, and commit generated files.
- **Validation:** `make validate` && `make verify-reproducible` exit 0; CI green.
- **Rollback:** revert the commit (additive metadata, nothing deployed).
- **Exact-target action:** seal `ESP32_API_KEY` or the OTA password only for an explicit
  credential-delivery request; validate the `.7.x` reservation against the network registry.
...
- **Validation:** `kustomize build` (v5.4.3) of every overlay passes `kubeconform -strict
  -ignore-missing-schemas`; images build locally; ingestor image makes an ESP32 connection in
  a **controlled** test (NOT the live device unless a controlled probe).
- **Rollback:** revert the commit or leave images unpromoted; VM unchanged.
- **Exact-target action:** open a *live* ESP32 connection only from an explicit live-device
  test request after proving no second writer; preserve unrelated shared-infrastructure work.

### P4 — CI/CD wire (Actions → ghcr)
- **Steps:** add `container-publish.yml` (reusable-build call) + `k8s-manifests.yml`; keep
  `ci.yml` as the unit validation.
- **Validation:** push to main produces `sha-<gitsha>` images in ghcr; manifest validation green; baked
  SHA shows at `/health/detailed`.
- **Rollback:** disable the workflow; nothing consumes the images yet.
- **Exact-target action:** **[SAFEGUARD: exact-target execution]** if any workflow would touch the firmware OTA path (it
  must not).
...
- **Steps:** add `platform/gitops/applications/local-staging/verdify.yaml` (copy
  ...
  secret(s).
- **Validation:** ArgoCD reconciles; pods Healthy in `verdify-staging` (empty DB initially); secrets
  present (sealed, not plaintext).
- **Rollback:** revert the image pin / delete the Application; k3s side only, VM untouched.
- **Exact-target action:** **[SAFEGUARD: exact-target execution]** namespace creation; first secret-sync run.
...
- **Validation:** portal (https://backstage.vallery.net) shows the Verdify component(s) + live k8s
  status + entity graph + owner + system.
- **Rollback:** remove `catalog-info.yaml` / revert the render. **Exact-target action:** n/a.
...
- **Validation:** endpoints reachable on `.7` behind Authentik; identity headers not spoofable; DB
...
- **Exact-target action:** **[SAFEGUARD: exact-target execution]** before pointing any *live device write* (a real setpoint
...
- **Validation:** Remote-SSH attach works; dev happens in the pod; worktree survives a pod restart.
- **Rollback:** flip `placement.mode` back to `vm`; VM worktrees still exist.
- **Exact-target action:** before retiring VM tmux lanes or worktrees, resolve each exact
  target, preserve unique commits, and retain a rollback/archive ref.

### P9 — Controlled cutover (the careful end)
...
- **Validation:** the full §8 DoD, each item independently proven.
...
- **Exact-target action:** stop each source service only within the explicit cutover request,
  after the validation passes and its restart command is staged. **The firmware OTA path is
...
- Commit repository changes directly and validate them in their owning repositories. ArgoCD
  remains the delivery mechanism for declarative cluster state.
- Cluster apply, ArgoCD sync, secret delivery, device-VLAN/routing changes, a first live
  setpoint from a pod, source-service stops, and firmware flashes all require an explicit
  exact-target request, technical preflight, staged rollback, and post-change verification.
