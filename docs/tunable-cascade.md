...
**Generated public reference:** `/reference/ai-tunables/`, generated by
`scripts/generate-ai-tunables-page.py` from the registry, MCP contracts,
ingestor maps, firmware source, and live DB landing evidence.
...
### Resolved 2026-05-12 — VPD dehum overshoot and heat staging invariants

Live data after the new tunables surface showed two consistency gaps:
`DEHUM_VENT` could keep running through dwell/min-runtime while VPD had already
crossed above `vpd_high`, and relay min-on timers could leave heat2 physically
on after heat1 or air-exchange demand had cleared.

Fixes now enforced across firmware and monitoring:

- `DEHUM_VENT` dry overshoot seeds the VPD watch timer and exits immediately.
  Cooling demand wins as VENTILATE with vent-mist assist; otherwise the
  controller can enter SEALED_MIST for bounded recovery.
- ESPHome relay execution suppresses non-safety heat whenever vent/fan air
  exchange is physically active.
- Heat2 is invalid without heat1. The executor forces heat2 off when heat1 is
  unavailable, and the replay invariant suite now has a `heat2_requires_heat1`
  check.
- The dispatcher keeps at least a 0.55 kPa house VPD deadband by relaxing the
  low side of the control band when crop/zone targets would otherwise produce
  chatter.
...
| Param | Type | Pydantic | DB | Dispatcher route | FW struct | FW use | cfg_* readback | Default | Valid range | Owner | Cascade |
|---|---|---|---|---|---|---|---|---|---|---|---|
| `mister_engage_kpa` | num | `PlanTransition.params`, `SetpointChange` | `setpoint_changes` | `number.vpd_mister_engage_kpa` | ESPHome global `mister_engage_kpa` | Physical S1 mister permissive once SEALED_MIST or explicit VENTILATE assist creates humidity demand; SEALED_MIST entry comes from `vpd_high` + `vpd_watch_dwell_s`. Zone stress can choose pulse target or satisfy S1 stress, but cannot create a standalone mister mode. | `cfg_mister_engage_kpa` | 1.6 | [0.5, 2.5] | planner | immediate |
| `mister_all_kpa` | num | `PlanTransition.params`, `SetpointChange` | `setpoint_changes` | `number.vpd_mister_all_kpa` | ESPHome global `mister_all_kpa` | Physical all-zone mister rotation threshold; firmware mist-stage S2 still follows `vpd_high_eff` + `mister_all_delay_s`. | `cfg_mister_all_kpa` | 1.9 | [1.0, 2.5] | planner | immediate |

During live VPD-high or near-edge `VENTILATE` stress with healthy dew-point
margin, these thresholds must stay coupled to the active house band. The
...
| `summer_vent_min_runtime_s` | num | `SetpointChange` only; not Tier 1 | `setpoint_changes` | `number.summer_vent_min_runtime_s` | `Setpoints.summer_vent_min_runtime_s` | **Reserved/no-op today**: clamped/read back but not consumed by `determine_mode()`. Do not rely on it until firmware implements the dwell. | `cfg_summer_vent_min_runtime_s` | 180 s | [60, 600] | firmware-reserved | no control impact |
...
The generated AI Tunables Traceability page now carries the exhaustive row-level
analysis for every registry and readback-only tunable. The sections below are a
compact historical grouping only; update the registry and regenerate the page
for authoritative defaults, ranges, firmware evidence, readbacks, and live-use
data.

### Mister pulse + timing
- Deprecated/no-op legacy duty-cycle params, not planner-pushable: `mister_on_s`, `mister_off_s`, `mister_all_on_s`, `mister_all_off_s`, `mister_max_runtime_min`
...
- Dry-stress override: `sw_direct_wet_stress_override_enabled`, `direct_wet_stress_vpd_margin_kpa`, `direct_wet_stress_min_dew_margin_f` (`direct_wet_stress_latest_hour` retired in wire schema v2, #588 — zero firmware presence)
...
- `summer_vent_min_runtime_s` is clamped/read back in `Setpoints` but not consumed by the summer-vent gate.
