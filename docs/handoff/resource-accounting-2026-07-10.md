...
Issue [#437](https://github.com/VerdifyConsultancy/verdify-platform/issues/437) is implemented through integrity-fix heads `4b27f47` (schema) and `23340fe` (consumers). This is a code/data-contract handoff only: no production migration, service restart, dashboard rollout, or device action occurred.
...
A read-only production-shaped local replay copied 107,467 water samples, 97,033 equipment-state rows, and 21,080 energy samples. After the adversarial run-count fix, a from-zero catch-up created or updated 20,544 event rows in 54.4 seconds; an immediate rerun processed zero rows in 0.17 seconds. The increased one-time catch-up cost counts relay episodes rather than only distinct relay names; steady-state work remains incremental. Recent complete-day conservation error is exactly zero. Representative July 8 output remains 147 accepted gallons = 9 single-run attributed + 136 ambiguous + 2 manual/unattributed; wall irrigation/fertigation was zero, matching the disabled/no-delivery baseline. The daily query completes in about 0.3 seconds. Energy reconciliation completes in about 0.03 seconds and keeps the runtime model bounded and the partial meter independently covered.

Both migrations classify safe-to-wrap, pass explicit outer rollback proofs, rerun idempotently, and pass fixed SQL fixtures. The generated schema restores cleanly. The expanded fixture proves incomplete-watermark, repeated-run, fertilizer-master/commissioning, contradictory-source, historical-revision, full/partial/modeled-only/stale/unavailable energy, and no-free-resource-score cases. Ruff, 249 focused contract tests, five targeted disposable-schema tests, dashboard JSON/ConfigMap parity, compilation, and `git diff --check` pass. The laptop-wide `make test` result is 715 passed, 141 failed, 6 skipped, 10 errors; failures are the inherited retired laptop/live-service assumptions plus three DB-backed assertions that pass against the disposable restored schema, not focused lane regressions.

The required independent critic returned `REQUEST_CHANGES` at `722a90c`. Every P1/P2 finding and both actionable follow-ups are addressed at `23340fe`; the item-by-item closure and re-review gate are in the lane `critic-report.md`.
