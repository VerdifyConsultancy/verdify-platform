...
4. **Dehumidification is a deterministic, outdoor-aware, bidirectional moisture-exchange selector** — **supersedes §3 "exactly ONE fixed heat-assisted-dehum state."** Each cycle, from indoor and outdoor temp+RH (→ absolute humidity / dewpoint), the controller deterministically estimates the projected VPD-toward-target gain of (a) **venting** (exchange toward outdoor absolute humidity) and (b) **heating** (raise saturation capacity → lower RH → raise VPD), and selects the more effective action. **Heat and vent MAY co-run** when the estimate proves both move VPD toward target (bounded: heat1 only with the vent, never heat2; min-dwell; dew-margin veto). It is **bidirectional**: it also handles the **humid-outside / dry-inside** case — when VPD is too high (too dry) and outdoor absolute humidity exceeds indoor, **import moisture by venting** instead of running the foggers. This replaces both the `cold_dehum_allowed` brake and BC-5's unconditional co-run with one model-driven path; it is the sanctioned heat+air-exchange exception, now estimator-gated rather than a fixed state.
   ...
   **§6.4 addendum (#410, per the 2026-07-03 design review): the HELD-TEMP vent candidate.** Evaluating the vent gain only at the cooled mix temperature `T_mix` under-sells venting whenever cooling cancels drying (the verified summer-night regime: cooled gain ≈ −0.004, held gain ≈ +0.124 kPa on the review's prod numbers). The too-wet ladder is therefore: **(1) cooled vent** (legacy semantics, incl. the over-cool veto and the min-dwell co-run) → **(2) held vent** (`vent_plus_heat_hold`): the projected gain at the heat1-HELD current temperature (`VPD' = es(T_now) − e_mix`) clears the margin, gated on the `dehum_vent_hold_enabled` tunable (ships OFF; flag OFF is bit-identical to the pre-#410 ladder) AND an actual-temp floor `temp ≥ band_heat_target_f + heat_hysteresis` (the floor — not `vent_overcools` — is the hold candidate's cold-sag guard; `vent_overcools` stays computed/reported and still vetoes the cooled candidate) → **(3) closed-vent heat assist** → **(4) nothing effective**. A held selection sets `hold_required`: the heat1 co-run arms **without** the min-dwell (a dwell-delayed reheat would vent unheated — the over-cool bug in miniature) and reheats up to the served `temp_target` while venting; ENTRY into `DEHUM_VENT` additionally requires temp to have recovered `GH_DEHUM_HOLD_REENTRY_F` (1.0 °F, named mechanical anti-chatter constant) above the floor (continuation and the `vpd_min_safe` rescue are exempt; the rescue arms the hold-reheat on the same cycle it forces the vent). "Held temp" is a candidate selector, not a trajectory promise — heat1 offsets only ~half of continuous-vent loss (21-day overnight slopes), so the design duty-cycles on the measured-temp floor exit. heat1 is the ELECTRIC stage (cycling acceptable); heat2 (gas) never participates.
