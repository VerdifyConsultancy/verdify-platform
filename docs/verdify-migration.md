...
Prepared for Jason · 2026-06-07 · Owner model: **four swim lanes** (Cluster / Application / Observability / Networking). Old Iris/Root/Nexus/Jason agent owners are retired; direct root execution calls are consolidated in §5.
...
- ★ **Apply `verdify-prod` App (writer shape) — the M5 cutover.** HARD safety boundary. *#73, #86, #132, #27.*
...
**Platform blockers:** SoT decoupling (fleet runs stale self-sourced staging); flannel non-enforcement; empty `state-truth.md`; iSCSI SC + NFS PV require exact-target infrastructure execution; M5/M6 retain the §3.4 device-VLAN reachability validation and M4 48h proof, and cannot run until the prod DB is restored on iSCSI.
...
**Remaining (implementation):**
- ★ **#105 reconcile canonical ESP32_API_KEY before sealing** (live `.env` sha `127f85d0` vs esphome `api_encryption` sha `df2784f9`). Cutover-adjacent: a key skew silently breaks the single device connection at M5. Requires a privileged live read; must NOT trigger re-flash. *#105, #30, #104.*
...
- **Retire Cloud Run www path** (M2.3); `verdify-www/deploy.yml` still deploys Cloud Run, DNS still CNAMEs `ghs.googlehosted.com`. Also remove the dead Cloud Run **Deploy** workflow that reds CI every run. DEC-WWW execution safeguard. *#116, #88, #120.*
...
Hard gates marked 🔒 (direct root execution) and ⚠ (needs-live-confirm / live-vantage required).
...
2. 🔒 **GO/NO-GO on the sanctioned path** (verify the exact target and prerequisites ordering before Root touches the live cluster) + **confirm target cluster** (existing live vs planned VMs 320/321/322). *§5.*
...
10. 🔒 **§3.4 device-VLAN reachability validation** + ⚠ **#105 ESP32_API_KEY reconciliation** (live read; wrong key = silent break at M5). *Networking + Application.*
...
|---|----------|---------|----------------|
| J1 | **Controlled critical path + ordering** | Confirm M0 reads → repoint staging → dev → prod DB restore → M4 48h → M5 → M6 | Execute in the §4 order with each technical preflight and rollback check passing. |
...
★ = critical-path (blocks prod cutover or VM decommission). 🔒 = execution safeguard. ⚠ = needs-live-confirm.
