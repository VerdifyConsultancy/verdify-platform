...
def test_script_restores_only_the_bounded_latest_dump_and_runs_real_gates():
    ...
    fixture_228 = script.index("-f /work/db/migrations/tests/test-228-experiment-v2-recovery-generation-guard.sql")
    fixture_229 = script.index("-f /work/db/migrations/tests/test-229-experiment-v2-recovery-failure-boundary.sql")
    fixture_230 = script.index("-f /work/db/migrations/tests/test-230-experiment-v2-observation-window-bundle-join.sql")
    fixture_231 = script.index(
        "-f /work/db/migrations/tests/test-231-experiment-v2-emergency-recovery-zero-exposure.sql"
    )
    fixture_232 = script.index(
    ...
    final_assertions = script.index("DO $assertions$", fixture_233)
    assert (
        max(apply_positions)
        < ledger_gate
        < fixture_217
