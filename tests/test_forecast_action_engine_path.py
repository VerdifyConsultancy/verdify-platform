...
def test_forecast_launch_has_no_legacy_srv_interpreter():
    ...
    fn = next(
        ...
        if isinstance(n, ast.AsyncFunctionDef) and n.name == "forecast_action_engine"
    )
    literals = _string_literals(fn)
    assert not any("/srv/greenhouse/.venv" in s for s in literals), "engine launch still hardcodes the dead /srv interpreter"
    assert not any(
        "/srv/verdify/scripts/forecast-action-engine.py" in s for s in literals
...
def test_mcp_restart_hook_has_no_legacy_srv_interpreter():
    ...
    fn = next(
        n
        for n in ast.walk(tree)
        if isinstance(n, ast.AsyncFunctionDef) and n.name == "planning_heartbeat"
    )
    literals = _string_literals(fn)
    assert not any(
        "/srv/greenhouse/.venv" in s for s in literals
...
def test_engine_repo_root_resolves_beside_its_repo_imports():
    ...
    assert (engine_repo_root / "slack_config.py").is_file(), "engine's slack_config import won't resolve at its REPO_ROOT"
    assert (engine_repo_root / "verdify_schemas").is_dir(), "engine's verdify_schemas import won't resolve at its REPO_ROOT"
