...
- **Gate:** jason — prod migration apply (#293, rollback-wrap proof per CLAUDE.md, non-self-transactional, then REFRESH MATERIALIZED VIEW CONCURRENTLY mv_band_curve) AND the live device tunable push (#377, preflight no-critical-alerts, cfg_band_track_fraction=0 readback as confirm, predeclared rollback value 0.25)
- **Lanes:** L4-DATA (186 apply, Jason-gated), L1-CLIMATE (#377 float-flip, Jason-gated), L2-OBS (trial read), L6-HA (writer-absent alert live as safety net)
...
- **Gate:** jason — firmware OTA (both OTA-1 and OTA-2); each carries replay-diff (THRESHOLD_PCT override documented for the intentional divergence) + firmware-invariants + test-firmware artifacts in the PR; OTA-2 (heat_dehum) requires its replay heat1-only divergence + an outcome_kpi overnight-heat1 before/after
- **Lanes:** L1-CLIMATE (firmware OTAs, Jason-gated), L0-SCHEMA (cfg_* rows already landed in W0), L2-OBS (before/after cycling gate), L5-DEPLOY (cycling deploy gate NEW-D), L8-PRODUCT (#118 2nd-writer channel-separation assertion before grow-light OTA)
...
L6-HA (#382 iSCSI writer-wedge)  ->  L6-HA CNPG cutover (#245) AND any writer restart in W1/W2
      (#382 left the writer down ~10min on restart (iSCSI target-cap); it blocks #245 and means no ingestor/writer restart is scheduled lightly. The single-writer fence + any DB_HOST flip interlocks with #382 being resolved (Jason-gated NAS fix).)
...
|---|---|---|---|---|---|
| #377 | W1 | jason-device-tunable | S | Float-flip: band_track_fraction source default -> 0 (corridor-keep) + live Jason-gated trial | #293, #371, #324 |
| #299 | W2 | jason-ota | M | Mister dwell-guard: mister_min_on_s/mister_min_off_s + per-actuator max-cycles/hr governor | #377, #300 |
| #386 | W2 | jason-ota | M | Grow-light solar-window shoulder min-on bypass fix (post-#295 cycling regression) | #293, #300 |
| #367 | W3 | jason-ota | M | BC-8: settle ONE anti-chatter mechanism (arm the dwell gate OR consolidate to hysteresis-only) | #299, #377 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #383 | W3 | jason-ota | L | Carve out heat_dehum: gate it behind a cfg-backed opt-in tunable (default OFF) + fog/mister anti-ping-pong hold + own post-float OTA | #377, #327, #371, #293 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #368 | W3 | jason-ota | M | BC-9: sensor input integrity — jump/rate-of-change rejection + flatline (stuck-in-range) -> SENSOR_FAULT | #377 |
| #370 | W3 | jason-ota | M | BC-11: move heat<->air interlock into pure resolve_equipment + invariant; collapse THERMAL_RELIEF / unify fog-assist if replay-diff=0 | #367 |
| #14 | W3 | jason-ota | M | Firmware digital-twin coverage: keep replay_emit/twin source byte-identical across this lane's firmware changes (epic anchor) | #31 |
...
| #369 | W3 | jason-ota | S | BC-10: firmware-side dead-code purge (stripped wet-stress entities / benign setpoint_unconfirmed tail / residual suppression knobs) | #326, #361 |
| #361 | W3 | jason-ota | M | Reassess diurnal anchors ONLY after solar parity (#293) + float evidence (#377); delete inert bias_heat/cool, do not reintroduce target-distance | #293, #377, #371 |
| #323 | W3 | jason-ota | L | Settable zone-priority ranking + mister-router arbiter (Vanda>Cannabis>Lime>Pepper) | #324, #291, #299 |
...
- **Worktree:** `lane/data-db-solar` · **Gate:** Jason — prod migration · **Milestone:** Greenhouse Control Optimization
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #293 | W1 | jason-prod-migration | M | Apply migration 186 (NOAA solar phase parity) to prod + REFRESH band-curve matviews + record KPI regime break | #347 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #327 | W3 | jason-prod-sync | L | Land moisture-estimator telemetry source + drift guard; activate it in prod via gated ingestor promotion | #293, #347 |
...
#### S3 — _As a operator who promotes images to the single prod environment behind the device-write gate, I want the prod-promote pipeline race fixed, the Actions-PR-creation org blocker resolved (or explicitly Jason-gated), and the ArgoCD/GitOps cleanup (selective-sync bugs, SSA, the verdify-prod-dark rename) triaged with evidence and a safe plan, so that a quiet pipeline reliably opens a digests-only prod-promote PR that a human can merge, and the prod ArgoCD app is reviewable, drift-triaged, and renamed without orphaning the live greenhouse writer._
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #319 | W0 | none | S | Fix prod-promote migrate rebuild-on-every-publish race | — |
| #320 | W0 | jason-infra | S | Resolve Actions-cannot-create-PR org blocker for prod-promote | — |
| #317 | W2 | jason-prod-sync | M | ArgoCD: plain full syncs on verdify-prod-dark get rewritten to a stale 2-resource selective scope | — |
| #318 | W2 | jason-prod-sync | M | Make per-resource ServerSideApply durable for the large grafana dashboard CMs | #339 |
| #336 | W2 | jason-prod-sync | L | ArgoCD/GitOps cleanup epic: app manifests, staging/live-branch retirement, verdify-prod-dark rename | #317, #318 |
| #335 | W0 | none | L | EPIC: CI/CD and Promotion Hardening (umbrella) | #303, #319, #320, #322, #338 |
| #288 | W2 | jason-ota | L | EPIC: Deploy Enablement — agent k3s/DB access + firmware CI/OTA + secret sealing (umbrella) | #303, #304 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #382 | W3 | jason-infra | M | Close the iSCSI target-cap wedge: confirm DSM fix, revert ingestor-state emptyDir TEMP patch back to the durable PVC, verify remount | — |
| #394 | W3 | jason-infra | M | Out-of-band writer-absent + split-brain alert: ship the VerdifyESP32SplitBrain/NoWriter PrometheusRule live (HA-3.3 #241 wiring) | — |
| #395 | W3 | jason-infra | S | Prove the nightly pg_dump backup actually runs green + add a backup-freshness alert (HA-1.8 #233 verification gap) | — |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #218 | W3 | jason-prod-migration | L | Interim DB durability: enable WAL archiving to the existing verdify-db-dumps NFS + run a practiced PITR restore drill (no new iSCSI, no cutover) | L6-S1 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #396 | W3 | jason-prod-migration | L | HA-4.2 (#244): author the PROD CNPG cluster (1+2 sync + Barman WAL/PITR) alongside live verdify-db | #382, L6-S1 |
| #245 | W3 | jason-prod-migration | L | HA-4.3: gated atomic live-DB cutover to CNPG (quiesce-writer → flip DB_HOST → bake → decom) [GATED, riskiest] | #382, #244, #218, L6-S3 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #398 | W3 | jason-prod-migration | M | Data-truth hygiene: re-baseline stale soil_moisture_targets + zones.yaml seed rows ahead of the hardware probe moves | L0-SCHEMA |
| #298 | W3 | jason-hardware | L | Probe/topology re-baseline: lime tree, cannabis, hydro pot, homeless SEN0600 (hardware moves + per-position import) | #45 |
| #45 | W3 | jason-hardware | M | Run irrigation-feedback bring-up after south-1 probe repair + center feedback install | — |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #37 | W3 | jason-ota | S | Change firmware default irrig_center_start_hour 10 -> 6 (+ boot-clamp fallback) | — |
| #296 | W3 | jason-ota | L | Soil-feedback irrigation sub-FSM (slow hysteresis; over-watering is the failure mode) | #298, #45, #297, #37 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #350 | W3 | jason-hardware | M | EPIC L8: Irrigation, Fertilization, and Orchids — horticulture/control decision surface | #296, #297, #298, #45, #37 |
| #16 | W3 | jason-hardware | S | EPIC: Hardware backlog (operator-gated sensing/equipment installs) | — |
| #51 | W3 | jason-hardware | S | CO2 two-point field calibration against a reference NDIR | — |
| #52 | W3 | jason-hardware | M | SEA1: orchid dormancy phase + dormant lighting clamp + dry-down governance (fall/winter) | — |

### L8-PRODUCT — Product-Plane / Decommission / Auth
- **Worktree:** `lane/product-decommission-auth` · **Gate:** Jason — infra · **Milestone:** Enablement: Decommission & Auth
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #174 | W3 | jason-infra | S | Admin auth-rehome to global auth.vallery.net SSO — convert to coordination request | — |
| #175 | W3 | jason-infra | S | Retire botauth.verdify.ai dead backend — confirm no consumers, remove route | #174 |
| #337 | W3 | jason-infra | S | EPIC steward: Decommission/Auth/residual product-plane — drive residuals to closure | #118, #43, #174, #175, #177 |
...
| Issue | Wave | Gate | Eff | Title | Depends |
|---|---|---|---|---|---|
| #177 | W3 | jason-hardware | S | Confirm DEFERRED: internet-friendly device channel (MQTT/edge-gateway) — gate intact | — |
