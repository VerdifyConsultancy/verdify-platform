...
- Audit reference base: `gate2-shadow/a754e873`.
...
1. **Deployed source, image, and config.** Read the three workload objects and
   ...
   resolve to one registry digest; and verify the OCI configuration's
   `VERDIFY_GIT_SHA` is
   `a754e873e8c42bfbb05a4c808bdf023c6462691a`. Use the 64 digest hex as
   `selector.runtime_environment_sha256`. Also require the API, MCP, and
   ingestor pod-template config revisions to equal the same source revision
   before recording it as the packet's live `candidate.config_revision`.
...
3. **Running entity grid.** On the existing authenticated ESPHome native-API
   connection, perform only entity/service enumeration and current-state
   replay. Canonicalize all 48 registry fields in ascending wire-ID order with
   entity kind, object ID, minimum, maximum, and step (switches use null numeric
   bounds), then hash the canonical bytes. Every source route and readback must
   match the enumerated object. The production revision must have the form
...
4. **Commissioning state.** From one current authenticated state replay,
   require exactly one finite/on-grid value for each of the 48 canonical fields
   at one connection and writer generation. Normalize with
