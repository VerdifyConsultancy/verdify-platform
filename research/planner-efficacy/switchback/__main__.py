...
Subcommands mirror the Section 8.3 ceremony:
  ...
  commit-mapping  read an existing 32-byte mapping secret file and print the
                  publishable commitment hex (this tool NEVER generates the
                  secret; the witnessed operating-system CSPRNG ceremony does)
...
def main(argv: list[str] | None = None) -> int:
    ...
    commit.add_argument(
        "--secret-file", required=True, help="file with exactly 32 raw secret bytes (never generated here)"
    )
